...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  SUBROUTINE map_set(proj_code,lat1,lon1,knowni,knownj,dx,nx,ny,stdlon,truelat1,truelat2,proj)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  ! to the cartesian x/y on a Lambert Conformal projection.
    ...
    IF (proj%hemi .EQ. -1.) THEN
      ...
      j = 2. - j
    ENDIF
!   write(702,*) deltalon, lon, proj%stdlon, proj%cone, i, j, rm
    RETURN
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   SUBROUTINE calc_map_factor(proj,lat,mf)
   IMPLICIT NONE
   TYPE(proj_info), INTENT(IN)  :: proj
   REAL, INTENT(in)  :: lat
   REAL, INTENT(out) :: mf ! Map Factor
   
   REAL(KIND=8) :: l, theta1, theta, k
   
!   l=SQRT((proj%polei*proj%dx)**2+(proj%polej*proj%dy)**2)
   k=proj%cone
   IF(proj%hemi==1)THEN ! north
      theta1=(90.-proj%truelat1)*rad_per_deg
      theta =(90.-lat)*rad_per_deg
   ELSE
      theta1=(90.+proj%truelat1)*rad_per_deg
      theta =(90.+lat)*rad_per_deg
   ENDIF
   ...
   CASE (PROJ_LATLON)
      mf=1
   CASE (PROJ_MERC,PROJ_PS,PROJ_LC)
      mf=(sin(theta1)/sin(theta))*((tan(theta/2)/tan(theta1/2))**k)
   CASE DEFAULT
      PRINT '(A,I2)', 'Unrecognized map projection code: ', proj%code
      STOP 'calc_map_factor'
   END SELECT
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   !----------------------------------------------------------------------------
   ! rotate U/V from model to lat/lon corrdinate
   SUBROUTINE rotate_uv ( proj, lon , u , v )
   IMPLICIT NONE
!   
      TYPE(proj_info), INTENT(IN)    :: proj
      REAL,            INTENT(IN)    :: lon
...
!      REAL, PARAMETER                           :: conv = 180.0 / pi
      ...
      INTEGER                         :: code
      REAL                            :: angle , cone , u_new , v_new
!      
!     WRITE(*,*) lon, code 
      code = proj % code

      IF ( code == PROJ_LATLON .OR. &
           code == PROJ_MERC   ) THEN
         RETURN             ! u/v just on lat/lon direction
      ENDIF

      IF( code == PROJ_PS ) cone =1.
      IF( code == PROJ_LC ) cone = proj % cone

      angle = lon - proj%stdlon

      IF( angle >  180.) angle = angle -360.
      IF( angle < -180.) angle = angle +360.

      angle = -angle * cone * radians_per_degree

!     WRITE(*,*) cone, angle 

      u_new = u *COS( angle )- v *SIN( angle )
      v_new = u *SIN( angle )+ v *COS( angle )
